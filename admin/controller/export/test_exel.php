...
{   ++$j;
 ...
 $sheet->getStyle('A'.($j).':F'.($j))->getBorders()->getTop()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
$sheet->getStyle('A'.($j).':F'.($j))->getBorders()->getBottom()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
$sheet->getStyle('A'.($j).':F'.($j))->getBorders()->getLeft()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
$sheet->getStyle('A'.($j).':F'.($j))->getBorders()->getRight()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
...
//        $sheet->getRowDimension($j)->setOutlineLevel($val->name_category);
        ...
        $sheet->setCellValueByColumnAndRow( 4 ,$j,$val->items[$i]['title']);
        $sheet->setCellValueByColumnAndRow( 5 , $j, "Перейти");
        $sheet->getStyle('F'.($j))->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
 $sheet->getStyle('F'.($j))->getFont()->getColor()->setRGB('081bf8');
 $sheet->getStyle('F'.($j))->getFont()->getColor()->setRGB('081bf8');
        $sheet->getRowDimension($j)->setOutlineLevel($val->levl+1);
// $sheet->getRowDimension($j)->setOutlineLevel(3);
//
$sheet->getCell('F'.($j))->getHyperlink()->setUrl( 'http://garantmarket.by/catalog/'.$val->items[$i]['categ_chpu'].'/'.$val->items[$i]['catal_chpu']);
    }
}
//echo $j;
